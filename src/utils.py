...
def download_mnist(data_dir='data'):
    """
    Télécharge le dataset MNIST avec plusieurs miroirs de secours

    Args:
        data_dir (str): Répertoire où sauvegarder les fichiers
    """
    # Liste de miroirs (essayés dans l'ordre)
    mirrors = [
    ...
    for key, filename in files.items():
        ...
        if not os.path.exists(filepath):
            ...
            # Essayer chaque miroir jusqu'à ce que l'un fonctionne
            success = False
            for mirror in mirrors:
                try:
                    url = mirror + filename
                    request.urlretrieve(url, filepath)
                    print(f"✓ {filename} téléchargé depuis {mirror}")
                    success = True
                    break
                except Exception as e:
                    print(f"  ✗ Échec depuis {mirror}: {str(e)[:50]}")
                    continue

            if not success:
                raise Exception(f"Impossible de télécharger {filename} depuis aucun miroir")
        else:
